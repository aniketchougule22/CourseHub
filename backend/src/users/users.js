...
router.post('/signup', async (req, res) => {
    try {
        ...
        if (create._id) {
            let token = generate_token.create_token(create._id);
            res.status(200).json({ status: true, statusCode: 200, message: "User singup successfully..!", data: create, token: token });
        } else {
    ...
    try {
        ...
        if (get != null) {
            ...
            if (pass) {
                ...
                res.status(200).json({ status: true, statusCode: 200, message: "User logged in successfully..!", data: get, token: token });
            } else {
                res.status(400).json({ status: false, statusCode: 400, message: "Invalid email or password..!" });
            }
        } else {
            res.status(400).json({ status: false, statusCode: 400, message: "Invalid email or password..!" });
        }
